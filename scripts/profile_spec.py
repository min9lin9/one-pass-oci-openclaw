...
def get_profile(name: str) -> Profile:
    if name not in PROFILES: raise StackError('Unknown profile; use operations, planning, or development')
...
def auth_mode(cfg: dict, name: str) -> str:
    ...
    default = 'chatgpt' if name == 'operations' or not cfg.get('OPENCODE_API_KEY') else ('opencode-go' if catalog == 'go' else 'opencode')
...
def config_for(p: Profile, cfg: dict, token: str) -> dict:
    ...
    if p.name=='operations': allowed += ['message','memory_search','memory_get']
